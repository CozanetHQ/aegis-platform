...
// SECURITY: was an unenforced "Bearer <anything>" check on a live feed of
// the most recent events across the WHOLE platform. Fixed to requireAdmin().
...
export async function GET(request: Request) {
  try {
    ...
    const events = await AuditEngine().getRecentEvents.execute(limit);
    return Response.json({ data: events.map(e => e.toPublicJSON()) });
  } catch (err) {
